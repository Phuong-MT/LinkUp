    ...
    <>
      <article className="rounded-xl bg-white p-4 shadow-xs border border-zinc-200/50 dark:bg-zinc-900 dark:border-zinc-800/50 transition-colors animate-fade-in">
        ...
        <p
          ...
          className="text-sm text-zinc-800 dark:text-zinc-200 leading-relaxed mb-3 whitespace-pre-wrap cursor-pointer"
        >
          {post.content}
        </p>
      ...
      {showDetailModal && (
        <PostDetailsModal post={post} onClose={() => setShowDetailModal(false)} onLike={onLike} />
      )}
    </>
  );
};
